...
const e = require("./lib/main"),
  ...
  a = o.app;
let l = process.cwd(),
  c = process.argv.map((e) => t.resolve(l, e)).indexOf(a.getAppPath());
-1 !== c && process.argv.splice(c, 1),
      ...
      e
        ...
        .options({
          ...
          detail: {
            ...
            default: !1,
            desc:
              "Run test in debug mode (It will not quit the test, and open the devtools to help you debug it).",
          },
          reporter: {
            type: "string",
            default: "dot",
...
a.on("window-all-closed", () => {}),
  ...
  a.on("ready", () => {
    ...
    if (
      ...
      if ((n.ensureDirSync(t.dirname(c.logfile)), n.existsSync(c.logfile)))
        try {
    ...
    ("test" !== c._command || c.detail) &&
      s.add(s.transports.Console, {
        level: "uncaught",
        formatter(e) {
          let o = "";
          void 0 !== e.message && (o += e.message),
            e.meta &&
        ...
        [
          ...
          (o) => {
            if (
              ...
              e.connectToConsole(),
              c.dev && "win32" !== process.platform && e.Debugger.startRepl(),
              !e.App.run)
            )
              return (
                e.error(
