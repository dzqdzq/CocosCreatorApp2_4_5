...
class f extends s {
  constructor(t, s) {
    ...
    switch (
      ...
      (this.save = s.save),
      "boolean" != typeof this.save && (this.save = !0),
      this.windowType)
    ) {
      case "dockable":
    ...
    if (
      ((this.nativeWin = new o(s)), void 0 === s.x && void 0 === s.y && f.main)
    ) {
      let t = e.screen.getDisplayMatching(f.main.nativeWin.getBounds()),
        i = this.nativeWin.getSize(),
        n = 0.5 * (t.workArea.width - i[0]),
        s = 0.5 * (t.workArea.height - i[1]);
      (n = Math.floor(n)),
        (s = Math.floor(s)),
        n < 0 || s < 0
    ...
    this.hideWhenBlur && this.nativeWin.setAlwaysOnTop(!0),
      this.nativeWin.on("focus", () => {
        y.focused || ((y.focused = !0), y.emit("focus"));
      }),
      this.nativeWin.on("blur", () => {
        setImmediate(() => {
          o.getFocusedWindow() || ((y.focused = !1), y.emit("blur"));
        }),
          this.hideWhenBlur && this.nativeWin.hide();
      }),
      this.nativeWin.on("close", (e) => {
        (this.closing = !0),
          "quick" === this.windowType &&
      ...
      this.nativeWin.webContents.on("will-navigate", (t, i) => {
        t.preventDefault(), e.shell.openExternal(i);
      }),
      f.addWindow(this);
  }
  dispose() {
  ...
  load(e, n) {
    ...
    if (!s)
      return (
        W.error(`Failed to load page ${e} for window "${this.name}"`), void 0
      );
    (this._url = e), (this._loaded = !1);
    let o = n ? encodeURIComponent(JSON.stringify(n)) : void 0;
    if (i.existsSync(s))
      return (
        (s = t.format({ protocol: "file", pathname: s, slashes: !0, hash: o })),
  ...
  forceClose() {
    (this._loaded = !1),
      f._saveWindowStates(),
      this.nativeWin && this.nativeWin.destroy();
  }
  focus() {
  ...
  openDevTools(e) {
    (e = e || { mode: "detach" }), this.nativeWin.openDevTools(e);
  }
  closeDevTools() {
  ...
  adjust(t, i, n, s) {
    let o = !1;
    "number" != typeof t && ((o = !0), (t = 0)),
      "number" != typeof i && ((o = !0), (i = 0)),
      ("number" != typeof n || n <= 0) && ((o = !0), (n = 800)),
      ("number" != typeof s || s <= 0) && ((o = !0), (s = 600));
    ...
    if (
      ...
      let e = a.workArea,
        ...
        d = e.x + (e.width - r),
        h = e.y + (e.height - r);
      (t + n <= s || t >= d || i <= l || i >= h) && (o = !0);
    }
    o ? this.nativeWin.center() : this.nativeWin.setPosition(t, i);
  }
  resetLayout(e, t) {
    let n,
      s = c.url(e);
    ...
    try {
      ...
      c.error(`Failed to load default layout: ${e.message}`), (n = null);
    }
    n && (b._closeAllSessions(), this.send("editor:reset-layout", n, !0, t));
  }
  emptyLayout() {
    b._closeAllSessions(), this.send("editor:reset-layout", null);
  }
  _send(...e) {
  ...
  _sendToPanel(e, t, ...i) {
    if ("string" != typeof t)
      return (
        W.error(`The message ${t} sent to panel ${e} must be a string`), void 0
      );
    let n = b._popReplyAndTimeout(i, b.debug);
    if (!n)
      return (
        (i = ["editor:ipc-main2panel", e, t, ...i]),
        !1 === this._send.apply(this, i) &&
          W.failed(
    ...
    return (
      ...
      (i = [
        ...
        t,
        ...i,
        b.option({ sessionId: s, waitForReply: !0, timeout: n.timeout }),
      ]),
  ...
  send(e, ...t) {
    ...
    return (
      ...
      (t = [
        ...
        e,
        ...t,
        b.option({ sessionId: n, waitForReply: !0, timeout: i.timeout }),
      ]),
  ...
  popupMenu(e, t, i) {
    void 0 !== t && (t = Math.floor(t)), void 0 !== i && (i = Math.floor(i));
    let n = this.nativeWin.webContents,
      s = new g(e, n);
    s.nativeMenu.popup(this.nativeWin, t, i), s.dispose();
  }
  get isMainWindow() {
    return f.main === this;
  }
  ...
  static find(e) {
    ...
    if (e instanceof o) {
      for (let t = 0; t < l.length; ++t) {
        let i = l[t];
        if (i.nativeWin === e) return i;
      }
      return null;
    }
    for (let t = 0; t < l.length; ++t) {
      let i = l[t];
      if (i.nativeWin && i.nativeWin.webContents === e) return i;
    }
    return null;
  }
  static addWindow(e) {
  ...
  static removeWindow(e) {
    let t = l.indexOf(e);
    if (-1 === t) return W.warn(`Cannot find window ${e.name}`), void 0;
    l.splice(t, 1);
  }
  static getPanelWindowState(e) {
    if (d) {
      let t = d.get(`panels.${e}`);
      if (t) return { x: t.x, y: t.y, width: t.width, height: t.height };
    }
    return {};
  }
  ...
  static saveLabelWidth(e, t) {
    d && (d.set(`panelLabelWidth.${e}`, t), d.save());
  }
  static _saveWindowStates(e) {
    if ("test" === c.argv._command) return;
    if (!f.main) return;
    if (!d) return;
    d.set("version", a);
    let t = d.get("panels") || [],
    ...
    d.set("windows", i), d.set("panels", t), d.save(), e && e();
  }
  static initWindowStates(e, t) {
    let i = require("../share/profile/default-layout-windows");
    m.load(t, i),
      (d = m.load(e)).get("version") !== a && ((i.version = a), d.set(null, i));
  }
        ...
        v.url(s.url) &&
          (s.main
            ? ((t.show = !1),
              ...
              (n = new f(e, t)),
              (f.main = n))
            : (n = new f(e, { show: !1, windowType: s.windowType })),
          "simple" === s.windowType && (n._panels = s.panels),
          !s.main &&
            s.panels &&
            s.panels.length &&
            n.nativeWin.setMenuBarVisibility(!1),
          n.adjust(s.x, s.y, s.width, s.height),
          s.main ? (n.show(), n.load(s.url)) : w.push({ win: n, state: s }));
      }
      if (f.main) return f.main.focus(), !0;
    }
    return !1;
  }
}
...
S.on("editor:ready", () => {
  ...
  S.on("editor:window-open", (e, t, i, n) => {
    let s = new f(t, (n = n || {}));
    s.nativeWin.setMenuBarVisibility(!1),
      n.width && n.height && s.nativeWin.setContentSize(n.width, n.height),
      s.load(i, n.argv),
      s.show();
  }),
    ...
    if (n.isMainWindow && !s) {
      ...
      if (i.existsSync(e))
        try {
          ...
          W.error(`Failed to load default layout: ${e.message}`), (s = null);
        }
    }
    e.reply(null, s);
  }),
  S.on("editor:window-save-layout", (e, t) => {
    let i = o.fromWebContents(e.sender),
      n = f.find(i);
    if (!n)
      return W.warn("Failed to save layout, cannot find the window."), void 0;
    (n._layout = t),
      f._saveWindowStates(() => {
        e.reply && e.reply();
      });
  ...
  S.on("editor:query-label-width", (e, t) => {
    ...
    if (!f.find(i))
      return W.warn("Failed to save layout, cannot find the window."), void 0;
    e.reply && e.reply(null, f.getLabelWidth(t));
  }),
  S.on("editor:window-focus", (e) => {
    let t = o.fromWebContents(e.sender),
      i = f.find(t);
    if (!i) return W.warn("Failed to focus, cannot find the window."), void 0;
    i.isFocused || i.focus();
  }),
  S.on("editor:window-load", (e, t, i) => {
    let n = o.fromWebContents(e.sender),
      s = f.find(n);
    if (!s) return W.warn("Failed to focus, cannot find the window."), void 0;
    s.load(t, i);
  }),
  S.on("editor:window-resize", (e, t, i, n) => {
    let s = o.fromWebContents(e.sender),
      a = f.find(s);
    if (!a) return W.warn("Failed to focus, cannot find the window."), void 0;
    n ? a.nativeWin.setContentSize(t, i) : a.nativeWin.setSize(t, i);
  }),
  ...
  S.on("editor:window-inspect-at", (e, t, i) => {
    ...
    n.inspectElement(t, i),
      n.devToolsWebContents && n.devToolsWebContents.focus();
  }),
  S.on("editor:window-remove-all-panels", (e) => {
    let t = o.fromWebContents(e.sender),
      i = f.find(t);
    if (!i) return e.reply(), void 0;
    i._removeAllPanels(), e.reply();
  });
