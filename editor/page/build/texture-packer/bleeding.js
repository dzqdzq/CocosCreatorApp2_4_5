          ...
          if (0 === i[s + 3])
            for (p = 0; p < g; p++)
              if (
                ...
                (e[s] = i[u]), (e[s + 1] = i[u + 1]), (e[s + 2] = i[u + 2]);
                break;
              }
  },
  i = function (e, i, t, o, f) {
    if (0 !== f.width && 0 !== f.height) {
      var r = f.y,
        ...
        y = 0;
      if (r - 1 >= 0)
        for (x = p + h, y = p + c + 3; x <= y; ++x) e[x - a] = i[x];
      if (l + 1 < o)
        for (x = g + h, y = g + c + 3; x <= y; ++x) e[x + a] = i[x];
      if (d - 1 >= 0)
      ...
      if (n + 1 < t)
        for (x = p + c, y = g + c; x <= y; x += a)
          (e[x + 4] = i[x]),
            ...
            (e[x + 7] = i[x + 3]);
      if (d - 1 >= 0 && r - 1 >= 0)
        for (y = (x = p + h) + 4; x < y; x++) e[x - a - 4] = i[x];
      if (n + 1 < t && r - 1 >= 0)
        for (y = (x = p + c) + 4; x < y; x++) e[x - a + 4] = i[x];
      if (d - 1 >= 0 && l + 1 < o)
        for (y = (x = g + h) + 4; x < y; x++) e[x + a - 4] = i[x];
      if (n + 1 < t && l + 1 < o)
        for (y = (x = g + c) + 4; x < y; x++) e[x + a + 4] = i[x];
    }
  };
module.exports = {
  applyBleed: function (t, o, f, r) {
    ...
    if (t.paddingBleed) {
      for (
        console.time("apply padding bleed"), l = 0, d = null;
        l < o.files.length;
        l++
      )
        (d = o.files[l].trim),
