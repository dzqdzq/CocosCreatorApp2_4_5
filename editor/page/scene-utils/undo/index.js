  ...
  undo() {
    ...
    for (let o = this.info.before.length - 1; o >= 0; --o) {
      let n = this.info.before[o],
        i = cc.engine.getInstanceById(n.id);
      try {
        e.restoreObject(i, n.data);
        let o = null;
        cc.Node.isNode(i) ? (o = i) : i instanceof cc.Component && (o = i.node),
          o && -1 === t.indexOf(o.uuid) && t.push(o.uuid),
    ...
    for (let o = this.info.before.length - 1; o >= 0; --o) {
      ...
      try {
        e.restoreNode(i, n.data),
          i && -1 === t.indexOf(i.uuid) && t.push(i.uuid),
          Editor.Selection.select("node", t);
      } catch (e) {
        Editor.error(`Failed to restore object ${i._name}: ${e}`);
      }
...
class d extends Editor.Undo.Command {
  ...
  redo() {
    ...
    for (let o = 0; o < this.info.list.length; ++o) {
      ...
      try {
        e.restoreDeleteNode(n.node, n.data), t.push(n.node.uuid);
      } catch (e) {
...
class s extends Editor.Undo.Command {
  ...
  static moveNode(e, t, o) {
    if (e.parent !== t) {
      let o = e.getWorldPosition(cc.v3()),
        n = e.getWorldRotation(cc.quat()),
        i = e.getWorldScale(cc.v3());
      (e.parent = t),
  ...
  undo() {
    ...
    for (let t = this.info.before.length - 1; t >= 0; --t) {
      ...
      s.moveNode(o.node, o.parent, o.siblingIndex), e.push(o.node.uuid);
    }
...
let a = [],
  ...
  g = {
    ...
    recordObject(t, o) {
      if ((o && m.setCurrentDescription(o), !f.some((e) => e.id === t))) {
    ...
    recordNode(t, o) {
      if ((o && m.setCurrentDescription(o), !a.some((e) => e.id === t))) {
        let o = cc.engine.getInstanceById(t);
        if (!o) return;
        try {
    ...
    recordCreateNode(e, t) {
      if ((t && m.setCurrentDescription(t), !p.some((t) => t.node.id === e))) {
    ...
    recordDeleteNode(t, o) {
      if ((o && m.setCurrentDescription(o), !h.some((e) => e.node.id === t))) {
        let o = cc.engine.getInstanceById(t);
        if (!o) return;
        try {
    ...
    recordMoveNode(e, t) {
      if ((t && m.setCurrentDescription(t), !u.some((t) => t.node.id === e))) {
        let t = cc.engine.getInstanceById(e);
        u.push({
          node: t,
          parent: t.parent,
    ...
    commit() {
      ...
      if (u.length) {
        ...
        m.add(s.type, { before: e, after: t }), (u = []);
      }
      h.length && (m.add(r.type, { list: h }), (h = [])), m.commit();
    },
    cancel() {
      (p = []), (h = []), (u = []), (f = []), m.cancel();
    },
    undo() {
      m.undo(), cc.engine.repaintInEditMode();
    },
    redo() {
      m.redo(), cc.engine.repaintInEditMode();
    },
    save() {
      m.save();
    },
    dirty: () => m.dirty(),
    on(e, t) {
      this._listeners.push({ type: e, listener: t }), m.on.apply(m, arguments);
    },
...
(module.exports = g), (module.exports._undo = m);
