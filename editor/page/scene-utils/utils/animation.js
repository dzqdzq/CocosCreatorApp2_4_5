...
let t,
  ...
  l = function (e, n) {
    if (e._prefab && e._prefab.fileId === n) return e;
    for (var t = e._children, i = 0, a = t.length; i < a; i++)
      if ((e = l(t[i], n))) return e;
    return null;
  },
  s = function (e, n) {
    if (e._id === n) return e;
    for (var t = e._children, i = 0, a = t.length; i < a; i++)
      if ((e = s(t[i], n))) return e;
    return null;
  },
  m = function (e, n) {
    if (!d.animation) return 0;
    let t = cc.engine
  ...
  u = function (e) {
    ...
    cc.assetManager.loadAny(t._uuid, async (n, i) => {
      if (n) return Editor.error(n), e && e();
      c.DIRTY = !1;
      let a = i.serialize();
      Editor.Ipc.sendToAll("scene:animation-clip-changed", {
        ...
        await I({ clip: i.name, data: a, dirty: !1 }),
        e && e();
    });
  },
  p = function (e) {
  ...
  g = function (e) {
    ...
    if (!n) return;
    let t = n.clip;
    if (!e || !t || e.is3DNode) return;
    let i = _Scene.gizmosView,
      a = "";
    if (e.uuid !== d.rNode) {
      a = e.name;
      let n = e.parent;
      for (; n.uuid !== d.rNode; ) (a = `${n.name}/${a}`), (n = n.parent);
      if (!n)
        return Editor.error("Can't generate child path for node."), void 0;
    }
    e.trajectoryGizmo ||
  ...
  E = function (e) {
    (c.PLAYING = !1), (_Scene.gizmosView.hidden = !1);
    let n = cc.engine
      .getInstanceById(d.component)
      .getAnimationState(d.animation);
    n &&
  ...
  y = function () {
    if (!d.animation || !d.component) return !1;
    _Scene.gizmosView.hidden = !1;
    let e = cc.engine
      .getInstanceById(d.component)
      .getAnimationState(d.animation);
    if (!e) return !1;
    ...
    e.pause(),
      cc.engine.setAnimatingInEditMode(!1, "timeline"),
      (c.PLAYING = !1),
      Editor.Ipc.sendToAll("scene:animation-state-changed", {
        clip: e.name,
        state: "pause",
  ...
  I = async function (n) {
    ...
    if (!t || n.clip !== d.animation) return;
    try {
    ...
    return new Promise((e, t) => {
      ...
      .then((e) => {
        let n = d.time;
        if ((n > e.duration && (n = e.duration), t.wrapMode !== e.wrapMode)) {
          t.getWrappedInfo(n);
          t.isPlaying && !t.isPaused
          ...
          i(t.wrapMode, cc.WrapMode.Reverse) && (n = Math.abs(n - e.duration));
        return t.setTime(n), Promise.resolve(e);
      })
      .then((e) => {
        ...
        for (let i in n._nameToState) {
          ...
          if (
            ...
            (e._uuid = e._uuid || o._uuid), (t = a);
            break;
          }
        }
        if (!t) return cc.error(`Can't find state from clip [${e.name}]`);
  ...
  A = function (e) {
    if (!c.DIRTY) return e && e();
    if (((c.DIRTY = !1), !d.animation)) return e && e();
    let n = cc.engine
      .getInstanceById(d.component)
      .getAnimationState(d.animation);
    if (!n) return e && e();
...
let S = function (e, n, t, i) {
  if (!i.properties || !i.properties[n] || Array.isArray(e)) return null;
  let a,
    o = i.properties[n],
    r = o.type;
  return (
    e && e.type && ((r = e.type), delete e.type),
    "animatable" in o && !1 === o.animatable
      ? null
      : !1 === o.visible || "cc.Node" === r
      ? null
...
module.exports = {
  ...
  contains: p,
  activate: function (e) {
    (d.aNode = e.uuid), c.RECORD && p(e) && g(e);
  },
  deactivate: function (e) {
    d.aNode && d.aNode === e.uuid && (d.aNode = null), f(e);
  },
  ...
  setAnimationTime: function (e) {
    if (!c.RECORD || !d.animation || !d.component) return !1;
    let n = cc.engine
      .getInstanceById(d.component)
      .getAnimationState(d.animation);
    return (
      ...
      (!c.PLAYING,
      (d.time = e),
      e > n.duration && (e = n.duration),
      (n.wrapMode & cc.WrapMode.Reverse) === cc.WrapMode.Reverse &&
        (e = n.duration - e),
      n.setTime(e),
      n.sample(),
      cc.engine.repaintInEditMode(),
  ...
  playAnimation: function () {
    ...
    let e = cc.engine.getInstanceById(d.component),
      n = e.getAnimationState(d.animation);
    if (!n) return !1;
    let t = n.getWrappedInfo(n.time);
    n.isPaused && t.time >= n.duration && n.setTime(0),
      e.play(n.name),
      n.on("stop", E),
      cc.engine.setAnimatingInEditMode(!0, "timeline"),
      (c.PLAYING = !0),
      Editor.Ipc.sendToAll("scene:animation-state-changed", {
        clip: n.name,
        state: "play",
  ...
  mountClip: function (e, n) {
    ...
    } else t = cc.engine.getInstanceById(d.component);
    if (!t) return !1;
  ...
  switchClip: function (e, t) {
    if (!e) return t && t(new Error("The clip's name cannot be empty."));
    let i = (e, n) => {
        if (!e || !d.rNode) return n && n();
        ...
        (d.animation = e), t.play(e), t.pause(e);
        let a = d.time;
        a > i.duration && (a = i.duration), i.setTime(a);
        let o = i.clip;
        if (!o) return n && n(new Error(`Animation switch failed - ${e}`));
        Editor.assetdb.queryUrlByUuid(o._uuid, (e, n) => {
          _Scene.updateTitle(n), (exports.title = n);
        }),
          i.sample(),
          g(),
  ...
  isRecording: function (e) {
    return (
      !!(c.RECORD && e instanceof cc.Animation) &&
  ...
  isRecordCurrentClip: function (e, n) {
    if (!(c.RECORD && e instanceof cc.Animation)) return !1;
    try {
  ...
  verifyPrefab: function (e, n) {
    ...
    try {
      let t = s(e, d.rNode),
        i = cc.instantiate(n.data);
      if (l(i, t._prefab.fileId).getComponent(cc.Animation)) return !0;
    } catch (e) {
      Editor.error(e);
    }
    return (
  ...
  onRevertPrefab: function (e) {
    c.RECORD || Editor.Ipc.sendToAll("scene:revert-prefab-changed", e);
  },
  onAddComponent: t,
  onRemoveComponent: i,
  onResetComponent: a,
  ...
  recordNodeChanged: function (e, n) {
    ...
    if (
      ...
      "string" == typeof t && (e = e.map((e) => cc.engine.getInstanceById(e)));
  ...
  queryEditProperties: function (e, n) {
    let t = [],
      ...
      a = e.types;
    return (
      n && t.push({ name: "active", type: "cc.Boolean" }),
      _.forEach((e) => {
        t.push(e);
      }),
      i.__comps__.forEach((e) => {
        if ("cc.Animation" !== e.type) {
          for (let n = 0; n < t.length; ++n)
            if (t[n].name.startsWith(e.type)) return;
          (function (e, n) {
            let t = [],
              i = e.type;
            if (!i) return Editor.warn("Type can not be null"), void 0;
            let a = n[i],
              o = e.value;
            for (let e in o) {
              if ("type" === e || "__scriptAsset" === e) continue;
              let n = S(o, e, i, a);
              null !== n && t.push({ type: o[e].type || null, name: n });
            }
            return t;
          })(e, a).forEach((e) => {
            t.push(e);
