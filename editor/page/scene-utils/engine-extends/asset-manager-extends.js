...
var u = (o.editorExtend = o.editorExtend || {});
function d(e) {
  o.releaseAsset(o.assets.get(e), true);
}
...
u.onAssetMoved = function (e, t, n) {
  ...
  var r = s.basenameNoExt(n);

  if (f.hasEventListener(e)) {
    u.queryAssetInfo(e, function (t, n, s) {
      if (!t) {
        d(e);

        if (!cc.js.isChildClassOf(s, cc.Scene)) {
  ...
  if ("undefined" != typeof _Scene) {
    u.queryAssetInfo(e, function (t, n, s) {
      if (!t) {
        if (cc.js.isChildClassOf(s, cc.Prefab)) {
          _Scene.walk(cc.director.getScene(), false, (t) => {
            var n = t._prefab;
            if (n) {
...
function g(e, s, a, r) {
  ...
  if (i.uuid && "folder" === i.type) {
    ...
    var u = Object.keys(n._path2uuid);
    let a = t.match(u, o);
    for (let e = 0; e < a.length; e++) {
      var d = n.assetInfoByPath(a[e]);

      if (l(d, s)) {
        c.push({ compressUuid: d.uuid, uuid: d.uuid });
      }
    }
  }
  return c;
