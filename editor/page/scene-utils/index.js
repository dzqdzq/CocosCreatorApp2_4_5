...
const e = require("async"),
  ...
  i =
    ...
    Editor.require("unpack://engine-dev/cocos2d/core/platform/CCClass")
      .getDefault,
    require("./edit-mode")),
  r = require("./lib/sandbox"),
  n = require("./lib/tasks"),
  o = require("./utils/prefab"),
  s = require("./utils/node"),
  a = require("./utils/animation"),
  c = require("./utils/effect"),
  d = require("./utils/scene"),
  l = Editor.require("packages://scene/panel/tools/camera"),
  u = Editor.Profile.load("global://settings.json");
let f = {
  ...
  init(e) {
    this.view ||
      ...
      this.gizmosView || (this.gizmosView = this.view.$gizmosView),
      this.Undo.init(),
    ...
    n
      ? e.waterfall(
          [
            r.loadScripts,
            (e) => {
              _Scene.reset(), cc.assetManager.editorExtend.loadJson(n, e);
            },
            (e, t) => {
              cc.director.runSceneImmediate(e.scene), t();
            },
  ...
  loadWorkspace(e, t) {
    if (e) {
      if (
        (Editor.Selection.select("node", e.sceneSelection, !0, !0),
  ...
  stashScene(e) {
    let t,
      r = i.curMode();
    if (r && r.getPreviewScene) t = r.getPreviewScene();
    else {
      var n = cc.director.getScene();
      let e = new cc.SceneAsset();
      (e.scene = n),
        (e.name = n.name),
        (t = Editor.serialize(e, { stringify: !0 }));
    }
    var o = this.getEditingWorkspace();
    (o.sceneJson = t), (Editor.remote.stashedScene = o), e && e(null, t);
  },
  _applyCanvasPreferences(e, t) {
    if (!(e = e || cc.Canvas.instance)) return;
    let i = Editor.Profile.load("project://project.json"),
      r = i.get("design-resolution-width"),
      n = i.get("design-resolution-height");
  ...
  adjustNodePosition(e, t) {
    void 0 === t && (t = Editor.Math.numOfDecimalsF(1 / this.view.scale)),
      e.setPosition(
  ...
  walk(e, t, i) {
    if (e) {
      if (!i) return Editor.warn("walk need a callback"), void 0;
      if (t) {
        if (i(e)) return;
      }
      (function e(t, i) {
  ...
  createPrefab(e, i) {
    let r = cc.engine.getInstanceById(e),
      n = o.createPrefabFrom(r),
      s = t.join(i, r.name + ".prefab"),
      a = Editor.serialize(n);
    Editor.Ipc.sendToMain("scene:create-prefab", s, a, (e, t) => {
      if (e) return Editor.error(e), o.unlinkPrefab(r), void 0;
      (n._uuid = t), u.get("auto-sync-prefab") && o._setPrefabSync(r, !0);
    });
  },
  _doApplyPrefab(e, t) {
    if (!e._prefab.asset) return;
    ...
    let r = o.createAppliedPrefab(e),
      n = Editor.serialize(r);
    t && this.revertPrefab(t.uuid, o.Deep.SelfAsset, r),
      Editor.Ipc.sendToMain("scene:apply-prefab", i, n);
  },
  applyPrefab(e, t, r) {
    let n = cc.engine.getInstanceById(e);
    if (!n || !n._prefab) return;
    ...
    let a = i.curMode();
    "prefab" === a.name && s.uuid === a.rootPrefabId && _Scene.Undo.save();
  },
  revertPrefab(e, t, i) {
    let r = cc.engine.getInstanceById(e);
    if (!r || !r._prefab) return;
  ...
  setPrefabSync(e) {
    ...
    t &&
      t._prefab &&
      ((t = t._prefab.root), o.setPrefabSync(t, !t._prefab.sync));
  },
  setGroupSync(e, t) {
  ...
  linkPrefab() {
    let e = {
      ...
      i = Editor.Selection.curActivate("node"),
      r = i && cc.engine.getInstanceById(i);
    if (!r) return Editor.Dialog.messageBox(t);
          ...
          !t &&
            ...
            ("prefab" === i.type
              ? cc.assetManager.loadAny(n, (e, t) => {
                  t &&
                    (r._prefab && o.unlinkPrefab(r._prefab.root),
                    o.linkPrefab(t, r),
                    r.name.endsWith(o.MISSING_PREFAB_SUFFIX) &&
                      o.setPrefabSync(r, !0, !0) &&
                      (r.name = r.name.slice(
                        0,
    ...
    if (t) {
      ...
      for (let e = 0; e < t._components.length; ++e) {
        ...
        if (cc.isValid(i) && i.constructor._executeInEditMode) {
          ...
          i.constructor._playOnFocus &&
            i.enabledInHierarchy &&
            cc.engine.setAnimatingInEditMode(!0, "component_playOnFocus");
        }
      }
    }
  },
  deactivate(e) {
    ...
    if (t && t.isValid) {
      if (!(n.runningTask && n.runningTask.run === this._softReload)) {
        ...
        if (e) {
          "prefab" === i.curMode().name || o.confirmPrefabSyncedLater(e);
        }
      }
      a.deactivate(t);
      for (let e = 0; e < t._components.length; ++e) {
        ...
        if (cc.isValid(i) && i.constructor._executeInEditMode) {
          ...
          i.constructor._playOnFocus &&
            i.enabledInHierarchy &&
            cc.engine.setAnimatingInEditMode(!1, "component_playOnFocus");
        }
      }
    }
  },
  async _syncPrefab(e, t) {
    ...
    if (
      ...
      if (await o.confirmPrefabSynced(r._prefab.root)) return t();
    }
    o.syncPrefab(e.uuid), t();
  },
  syncPrefab(e) {
    n.push({
      name: "sync-prefab",
  ...
  assetChanged(e) {
    if ((a.assetChanged(e), c.assetChanged(e), "prefab" === e.type)) {
      let t = i.curMode();
      "prefab" === t.name ? t.prefabAssetChanged(e) : this.syncPrefab(e);
    }
  },
  assetsMoved(e) {
    a.assetsMoved(e), c.assetsMoved(e);
  },
  assetsCreated(e) {
    c.assetsCreated(e);
  },
  ...
  setTransformTool(e) {
    this.gizmosView && (this.gizmosView.transformTool = e),
      cc.engine.isInitialized && cc.engine.repaintInEditMode();
  },
  setPivot(e) {
    this.gizmosView && (this.gizmosView.pivot = e),
      cc.engine.isInitialized && cc.engine.repaintInEditMode();
  },
  setCoordinate(e) {
    this.gizmosView && (this.gizmosView.coordinate = e),
      cc.engine.isInitialized && cc.engine.repaintInEditMode();
  },
  setGizmoDimension(e) {
    this.gizmosView && (this.gizmosView.is2D = e),
      cc.engine.isInitialized && cc.engine.repaintInEditMode();
  },
  isGizmoToolLocked() {
  ...
  lockGizmoTool(e) {
    this.gizmosView && (this.gizmosView.lockGizmoTool = e);
  },
  alignSelection(e) {
    ...
    let i = 1e10,
      ...
      n = -1e10,
      o = -1e10,
      a = (t = (t = t.map((e) => cc.engine.getInstanceById(e))).filter((e) => {
        let i = e.parent;
        for (; i; ) {
    ...
    a.forEach((t) => {
      ...
      this.adjustNodePosition(r, o), cc.engine.repaintInEditMode();
    }),
      this.Undo.commit();
  },
  distributeSelection: function (e) {
    ...
    i.forEach((t, i) => {
      ...
      this.adjustNodePosition(c, u), cc.engine.repaintInEditMode();
    }),
      this.Undo.commit();
  },
