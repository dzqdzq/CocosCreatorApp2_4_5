...
let t = {
      ...
      (this._queue = s.queue((s, t) => {
        let i = function (i) {
            i
              ...
              : this.silence || Editor.log(`Finish task ${s.name}`),
        ...
        try {
          this.silence || Editor.log(`Handling task ${s.name}`),
            s.run.apply(r, e);
        } catch (i) {
          this._handleError(s, i), t(i);
        }
      }));
  },
  stash() {
  ...
  push(s, t) {
    this.silence || Editor.log(`Push task ${s.name}`),
      this._stashArray ? this._stashArray.push([s, t]) : this._queue.push(s, t);
  },
  kill() {
    this._queue.kill();
  },
