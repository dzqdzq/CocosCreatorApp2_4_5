...
module.exports = {
      ...
      (await t("db://**/*", "folder")).forEach((e) => {
        ...
        if (e.isBundle || "db://assets/resources" === t) {
          ...
          (s.name = e.bundleName || r.basenameNoExt(t)),
            (s.url = t),
            "db://assets/resources" === t && (s.priority = 8),
            n.push(s);
        }
      }),
      n
  ...
  async queryBundlesWithScenes() {
    let s = {},
      t = this._getAssetDB(),
      n = e(t.queryMetas).bind(t),
      i = await this.queryBundleFolders(),
      a = await n("db://**/*", "scene"),
      l = [];
    return (
      (a = a.map((e) => {
        let r = Object.assign({}, e);
        return (r.url = t.uuidToUrl(r.uuid)), r;
      })).forEach((e) => {
  ...
  _getFilesInDirectory(e, t) {
    s.readdirSync(t).forEach((n) => {
      let i = r.join(t, n);
      s.statSync(i).isDirectory() ? this._getFilesInDirectory(e, i) : e.push(i);
    });
  },
  async compressDirs(e, i, a) {
    await new Promise((l) => {
      let u = new t(),
        ...
        d = [];
      e.forEach((e) => {
        this._getFilesInDirectory(o, e), d.push(e, r.join(e, "**/*"));
      }),
        o.forEach((e) => {
          let t = r.relative(i, e),
            n = r.join("res", t);
          (n = n.replace(/\\/g, "/")), u.file(n, s.readFileSync(e));
        }),
        u
          .generateAsync({
            type: "nodebuffer",
