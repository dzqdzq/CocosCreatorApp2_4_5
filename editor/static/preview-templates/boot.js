  ...
  function showSplash() {
    ...
    if (size.width < size.height) {
      ...
      splash.style.backgroundImage =
        'url("app/editor/static/img/logo_portrait.png")';
      splash.style.backgroundSize = "30%";
      marginTop = (size.height - size.width * (1 - LOGO_SIZE)) / 2;
    } else {
      var logoDisplayH =
        ((size.width * LOGO_SIZE) / LOGO_IMG_L_W) * LOGO_IMG_L_H;
  ...
  function checkEmptyScene() {
    ...
    if (scene) {
      ...
      if (scene.children.length === 1) {
        ...
        if (
          node._components.length > 0 &&
          !(node._components[0] instanceof cc.Canvas)
        ) {
          return;
        }
      }
