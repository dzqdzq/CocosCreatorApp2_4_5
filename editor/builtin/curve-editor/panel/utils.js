...
function t(n, t, e, a) {
  ...
  for (
    ...
    if (f[g])
      for (l = 4, m = []; l < f.length; l++) l % 4 !== g && m.push(f[l]);
  for (s = [], g = 0; g < 4; g++)
  ...
  const p = o * s[0] + T * s[1] + t * s[2] + a * s[3],
    y = o * s[4] + T * s[5] + t * s[6] + a * s[7],
    x = o * s[8] + T * s[9] + t * s[10] + a * s[11];
...
module.exports = {
  ...
  drawHermite: function (a, i, o) {
    ...
    const u = i.canvas.width,
      T = i.canvas.height;
    let g;
    i.clearRect(0, 0, u, T), (g = o ? T / u / 2 : T / u);
    const l = a.map((n) => ({
    ...
    for (let n = 0; n < l.length - 1; n++) {
      const a = l[n + 1],
        u = l[n],
        g = e(t(u.point, u.outTangent, a.point, a.inTangent));
      i.beginPath();
      for (let n = u.point.x; n <= a.point.x; n++)
        o ? i.lineTo(n, T - 0.5 * g(n) - T / 2) : i.lineTo(n, T - g(n));
      i.stroke();
    }
    o &&
      ((i.strokeStyle = "#ccc"),
