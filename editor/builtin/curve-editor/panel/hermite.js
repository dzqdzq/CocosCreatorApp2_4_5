const { clamp: t } = Editor.Math,
  { drawLine: i, calcHermite: e, calcFunc: n, Point: s } = require("./utils"),
  r = 2;
module.exports = class {
  get keyFrames() {
    return this._keyframes.map((t) => {
      const i = this.grid.axisToOri(t.point),
        { w: e, h: n } = this.grid.location;
      return {
        time: i.time,
        value: i.value,
  ...
  get ctrlKey() {
    return this._keyframes.map((t, i) => {
      ...
      0 !== i && (t.inPoint = e.inPoint),
        i !== this._keyframes.length - 1 && (t.outPoint = e.outPoint);
      const n = this.grid.axisToCanvas(t.point);
      return (t.point.canvas = n), t;
    });
  }
  constructor(t) {
    (this._keyframes = {}),
  ...
  rePaint() {
    this.clear(), (this.cxt2D.strokeStyle = this.curveConfig.strokeStyle);
  ...
  draw(t) {
    const i = this.cxt2D,
      { w: s, h: r } = this.grid.location;
    try {
      this._keyframes = JSON.parse(JSON.stringify(t));
    } catch (t) {
      return console.error(t), void 0;
    }
    this.negative ? (this.mult = r / s / 2) : (this.mult = r / s);
    for (const t of this._keyframes) {
      const i = this.grid.tranToAxis(t);
      (t.outTangent *= this.mult), (t.inTangent *= this.mult), (t.point = i);
    }
    this.hermiteArgs = [];
  ...
  delKeyFrame(t) {
    ...
    if (
      (i && this.hermiteArgs.splice(t, 1),
      n && this.hermiteArgs.splice(t - 1, 1),
      i && n)
    ) {
  ...
  update(t = this.cxt2D) {
    ...
    t === this.cxt2D && this.drawEdge();
  }
  moveY(i) {
    const { h: n } = this.grid.location,
      s = JSON.parse(JSON.stringify(this._keyframes));
    for (let r = 0; r < this._keyframes.length; r++) {
      const a = this._keyframes[r],
        { point: o } = a;
      if (o.y > n || o.y < 0) return (this._keyframes = s), !1;
      if (
        ((o.y += i),
        (o.y = t(o.y, 0, n)),
  ...
  moveKey(i, n, s) {
    const r = this._keyframes[s],
      a = this._keyframes[s + 1],
      o = this._keyframes[s - 1],
      { point: h } = r,
      { w: c, h: g } = this.grid.location;
    let l = [0, g];
    if (
      (this.negative && (l = [-g / 2, g / 2]),
      (h.x > c || h.x < 0) && (h.y > g || h.y < 0))
    )
      return;
  ...
  calcCtrl(t, i) {
    ...
    if ("outTangent" !== i) {
      ...
      (s = this.calcCtrlPoint(e, i, "inTangent")).canvas =
        this.grid.axisToCanvas(s);
    }
    if ("inTangent" !== i) {
      const i = this._keyframes[t].outTangent;
      (n = this.calcCtrlPoint(e, i, "outTangent")).canvas =
        this.grid.axisToCanvas(n);
    }
  ...
  drawEdge() {
    const { w: t, x: e } = this.grid.location,
      n = this.ctrlKey[0].point.canvas,
      s = this.ctrlKey[this._keyframes.length - 1].point.canvas;
    this.cxt2D.save(),
