...
function d(n, r) {
  (function n(s) {
    if (!e.existsSync(s)) return;
    const o = "node_modules" === t.basename(s),
      i = t.basename(s).startsWith(".");
    o ||
      i ||
      (r(s),
      e.statSync(s).isDirectory() &&
        e.readdirSync(s).forEach((e) => n(t.join(s, e))));
  })(n);
}
exports.genAssetsTree = function () {
  (s = {}), i.splice(0), c.splice(0);
  const n = (function () {
      const t = `${Editor.Project.path}/settings/serverless.json`;
      var n = "{}";
      e.existsSync(t) && (n = e.readFileSync(t));
      var r = JSON.parse(n);
      return r && r.env_id && "0" !== r.env_id ? r.env_id : "undefinedenv";
    })(),
    r = t.join(Editor.Project.path, "./serverless/cloud-function");
  d(r, (o) => {
    r !== o &&
      ((!e.lstatSync(o).isDirectory() &&
        [".js", ".ts", ".json", ".php"].indexOf(t.extname(o)) <= -1) ||
        (o.match(new RegExp(`[/|\\\\]+${n}([/|\\\\]+|$)`)) &&
          (s[o] = a(o, r))));
  });
  const u = t.dirname(r);
  d(u, (n) => {
    u !== n &&
      ((!e.lstatSync(n).isDirectory() &&
        [".js", ".ts", ".json"].indexOf(t.extname(n)) <= -1) ||
        (n.indexOf("mgobe-server") > -1 && (s[n] = a(n, u))));
  }),
    (o.children.length = 0);
  for (let e in s) {
    let c = s[e];
    (c.fold = l(e)),
      t.basename(e) === n && (c.name = "cloud-function"),
      c.hidden ||
        (c.parent === r || c.parent === u
          ? (o.children.push(c),
            (c.assetType = "mount"),
            (c.level = 0),
            (c.show = !0))
  ...
  return i.sort(Cache.sortChildrenNodes), this.sortAssetsTree(), o;
};
(exports.sortAssetsTree = function () {
  ...
  (exports.sortChildrenNodes = (e, t) => {
    ...
    return n || r
      ? n && r
        ? e.name.localeCompare(t.name, "en", { numeric: !0 })
        : n
        ? -1
        : 1
      : (0, e.name.localeCompare(t.name, "en", { numeric: !0 }));
  });
let a = function (e, n) {
  const r = t.extname(e),
    s = t.dirname(e),
...
(exports.createNode = a),
  (exports.add = function (e, t = !1) {
    ...
    if (n) {
      ...
      t < 0 && (t = ~t), n.children.splice(t, 0, e);
    } else o.children.push(e);
    return (e.show = t), i.push(e), r.emit("node-added", e), e;
  }),
  (exports.remove = function (e) {
    let t = this.queryNode(e),
      n = t.parent ? s[t.parent] : o;
    if (n) {
    ...
    delete s[e];
    let c = i.indexOf(t);
    return c > -1 && i.splice(c, 1), r.emit("node-removed", t), !0;
  });
