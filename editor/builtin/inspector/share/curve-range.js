...
Vue.component("cc-curve-range", {
  ...
  methods: {
    ...
    isOneCurve() {
      return (
        this.target.value.mode.value === cc.CurveRange.Mode.Curve &&
        (this._onCurveChanged(), !0)
      );
    },
    isTwoCurves() {
      return (
        this.target.value.mode.value === cc.CurveRange.Mode.TwoCurves &&
        (this._onCurveChanged(), !0)
      );
    },
    isTwoConstants() {
    ...
    refresh() {
      ...
      let e = !1;
      if (
        (-1 === this.target.min && (e = !0),
        this.mode === cc.CurveRange.Mode.Curve)
      ) {
        const n = this.getThumbCtx("thumb");
        n && t(this.target.value.curve.value.keyFrames.value, n, e);
      } else if (this.mode === cc.CurveRange.Mode.TwoCurves) {
        const n = this.getThumbCtx("thumbMin");
        n && t(this.target.value.curveMin.value.keyFrames.value, n, e);
        const a = this.getThumbCtx("thumbMax");
        a && t(this.target.value.curveMax.value.keyFrames.value, a, e);
      }
    },
    getThumbCtx(e) {
      if (!this.$els[e]) return console.warn(`${e} is not exit!`), null;
    ...
    showEditor(e) {
      if (this.readonly || this.target.readonly) return;
      let t = !1;
      -1 === this.target.min && (t = !0),
        n(
          {
