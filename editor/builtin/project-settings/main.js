...
(module.exports = {
  ...
  messages: {
    ...
    "save-simulator-config"(o, t, r) {
      ...
      e.existsSync(s) && e.writeJsonSync(s, r, "utf8");
    },
    "query-tab"(e) {
      e.reply(null, r);
    },
    ...
    if (
      (r ||
        (r = (function () {
          const t = require("path"),
            r = require("globby");
          let s,
            n = t.join(Editor.Project.path, "assets");
          try {
          ...
          return s && s.ver
            ? o.satisfies(s.ver, "< 2.3.0", { includePrerelease: !0 })
              ? (console.log(
      ...
      try {
        let t = Editor.Profile.load("project://project.json"),
          s = t.get("excluded-modules"),
          n = e.readJsonSync(Editor.url("unpack://engine/modules.json")),
          i = !1;
        n.forEach((e) => {
          !s.includes(e.name) &&
            e.since &&
            o.satisfies(r, "< " + e.since, { includePrerelease: !0 }) &&
            (s.push(e.name),
            console.log(
          ...
          i && (t.set("excluded-modules", s), t.save());
      } catch (e) {
        console.error(e);
      }
    }
