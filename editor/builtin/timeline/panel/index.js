...
const o = Editor.require(`packages://${i}/panel/component/home`),
  ...
  s = Editor.require(`packages://${i}/panel/libs/manager`),
  p = Editor.require(`packages://${i}/panel/libs/advice`);
Editor.Panel.extend({
  ...
  deleteTheSelectedKeys() {
    ...
    if (
      (e.forEach((e) => {
        -1 === t.indexOf(e.path) && t.push(e.path);
      }),
      (t = t.map(
  ...
  pasteTheCopiedKeys() {
    ...
    let t = this.vm.frame,
      i = this.vm.clip.id,
      a = this._copyCache.some(
        (e, t) => t > 0 && e.path !== this._copyCache[t - 1].path
      ),
      o = this._copyCache[0];
    if (
      !this._copyCache.some((t, i) => !(i < 0) && !e.contains(o.path, t.path))
    ) {
