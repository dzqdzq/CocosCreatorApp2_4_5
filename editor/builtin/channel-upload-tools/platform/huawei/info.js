...
module.exports = {
  ...
  async created() {
    this.loadHistory(),
      this.needUpload && (await this.prepareApi(), await this.uploadApk());
  },
  watch: {
    history: {
      deep: !0,
  ...
  methods: {
    ...
    returnClick() {
      this.cancelClick(), (this.page = `${o.PLATFORM}-upload`);
    },
    cancelClick() {
      (this.cancel = !0), this.checkCancel();
    },
    loadHistory() {
    ...
    addHistory() {
      this.history.length >= o.MAX_HISTORY_LENGTH && this.history.pop(),
    ...
    changeState(t) {
      ...
      (this.uploadState = t), (this.progressTips = this.t(e));
    },
    getStateKey(t) {
      for (let e in r) if (t === r[e]) return e;
    },
    formatTime: (t) =>
    ...
    checkCancel() {
      return (
        !!this.cancel &&
        ((this.uploadProgress = 0), this.changeState(r.cancel), !0)
      );
    },
    async prepareApi() {
      let t;
    ...
    async uploadApk() {
      if (((this.uploadProgress = 10), this.checkCancel())) return;
      (this.uploadProgress = 20), this.changeState(r.fetch_url);
      ...
      if (this.checkCancel()) return;
      (this.uploadProgress = 30), this.changeState(r.upload);
